...
function similar(a,b){
  ...
  const thTitle=sameCat?0.85:0.9
  const thContent=sameCat?0.8:0.85
...
function main(){
  ...
  const items=[]
  let attempts=0
  while(items.length<target && attempts<5000){
    attempts++
  ...
  console.log(`generated unique: total=${items.length} attempts=${attempts} -> ${out}`)
}
